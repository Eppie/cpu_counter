...
const CounterDefinition kCounters[] = {
    ...
    },
    {
        "core-active-cycle",
        "Core Active Cycle",
        "Cycles where the core is powered and running -- including memory-stall busy-wait -- as opposed to idle or blocked (e.g. waiting in a syscall).",
        "Counter-intuitively, a long dependent-load chain stays ~95% active (the core busy-waits on misses); only genuine idle/blocked time, like the syscall-heavy self-modifying demo (~11% active), shows up as inactive. Watch the active/total ratio.",
        Group::CoreExecution,
        Tier::Stable,
        PerfCounter::Named("CORE_ACTIVE_CYCLE"),
        "random-pointer-chase",
    ...
    },
    {
        "dtlb-access",
        "DTLB Access",
        "Data-side first-level TLB accesses (one per data memory access, hit or miss).",
        "This is an access count, not a miss count: every data access consults the L1 DTLB, so an access-heavy loop logs millions while a register-only loop logs almost none.",
        Group::TlbPageWalk,
        Tier::Stable,
